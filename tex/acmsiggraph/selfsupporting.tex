...
\shortcite{Maxwell64}, with the Airy stress function becoming a polyhdral 
surface directly related to a reciprocal force diagram. Our own 
...
We are discretizing a self-supporting surface by a polyhedral mesh 
$\SS=(V,E,F)$ (see Figure~\ref{fig:reciprocal}). Loads are again vertical, 
...
symbols $H,K$ instead of $H^\rel,K^\rel$. The formulas above together with 
the observation $\tr (M\Hess\phi)=\tr(E_2)=2$ immediately imply
	\begin{align*}
	K_s = \det \Hess s,
		\
	H_s = {\Delta s \over 2},
...
\begin{proof} It is sufficient to show the formula
	\begin{align*}
	2A_i(\SS,\Phi)
	= \sum_{j\sim i} w_{ij} (s_j-s_i).
 	\end{align*}
...
structure, because of its central part is concave. Our algorithm finds a 
...
reference surface or excessive tangetial sliding.
...
between the two pillars.
 ...
 \begin{figure}[t]
  ...
  \caption{A mesh with holes (left) requires large deformations to both 
the top view and heights to render it self\dash supporting 
...
\begin{figure*}[t]
\centerline{\begin{overpic}[width=0.24\textwidth]{fig/lilium-vf.jpg}
		\put(0,30){(a)}
	\end{overpic}\relax
	\begin{overpic}[width=0.24\textwidth]{fig/lilium-pq.jpg}
		\put(0,30){(b)}
	\end{overpic}\relax
	\begin{overpic}[width=0.24\textwidth]{fig/lilium-pq-n.jpg}
		\put(0,30){(c)}
	\end{overpic}\hfill
	\begin{minipage}[b]{.26\textwidth}
	\caption{Planar quad remeshing of the ``Lilium tower'' surface of 
Figure~\protect\ref{fig:Lilium}. (b) Principal directions. (c) Quad mesh 
guided by principal directions is close to planar and close to self\dash 
supporting. (c) Small changes make it self\dash supporting.}
 \label{fig:lilium:pq}\end{minipage}}

\centerline{\begin{overpic}[width=0.24\textwidth]{fig/cas-vf.jpg}
		\put(0,35){(a)}
	\end{overpic}\relax
	\begin{overpic}[width=0.24\textwidth]{fig/cas-pq.jpg}
		\put(0,35){(b)}
	\end{overpic}\relax
	\begin{overpic}[width=0.24\textwidth]{fig/cas-pq-n.jpg}
		\put(0,35){(c)}
	\end{overpic}\hfill
	\begin{minipage}[b]{.26\textwidth}
	\caption{Planar quad remeshing of the surface of 
Figure~\protect\ref{fig:cas}, analogous to 
Figure~\ref{fig:lilium:pq}.}\label{fig:cas:pq}\end{minipage}}
  \end{figure*}
...
self\dash supporting surface is possible. Taking an arbitrary non-planar 
...
Figure~\ref{fig:badpq}. Figures~\ref{fig:lilium:pq} and \ref{fig:cas:pq} 
...
We ask: Which perturbations $\SS+\RR$, having $z$ coordinates $z_i = 
s_i+r_i$, support the {\em same} vertical loads as $\SS$ does? Such a mesh 
must satisfy $\Delta_\phi(s+r)=\Delta_\phi s$, so $\Delta_\phi r = 0$. 
...
An interesting special case occurs if $\Phi$ is an \emph{isotropic Koebe 
mesh}, i.e., a PQ mesh whose edges touch the Maxwell paraboloid. Since 
$\Phi$ approximates the Maxwell paraboloid, we get 
$2K(\vw_i)H^{\rel}(\vw_i)$ $ \approx 1$ which means $\Phi$ is self\dash 
supporting for unit load. Applying the Christoffel dual construction 
described above yields a family a ``minimal'' mesh $\RR$ and a family of 
meshes $\Phi+\alpha\RR$ which are self\dash supporting for unit load. 
Figure~\ref{fig:enneper} shows an example.
