...
and \S\ref{sec:thrustnetworks} explain our mathematical
model and discretization. The connection
with isotropic geometry (\S\ref{sec:smooth} and \S\ref{sec:discrete})
is important for the later Section \ref{sec:special}.
Finally we discuss convergence \secref{sec:convergence}.
...
forces which is entirely contained within the structure.
  ...
  \begin{figure}[t]
  \centering
  \begin{overpic}[width=.85\columnwidth]{fig/reciprocal}
	\put(0,33){$\SS$}
	\lput(13,33){$\vw_i$}
...
polyhedron exist only locally. Our computations do not require global existence.
  ...
  \begin{figure}[t]
 \centerline{\vphantom{\includegraphics[width=.85\columnwidth]
...
\begin{figure}[h]
	\centerline{\begin{overpic}[width=.53\columnwidth]{fig/enneper_v.jpg}
		\color{gelb}
