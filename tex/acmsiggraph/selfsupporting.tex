...
increased robustness for slight smoothing of the resulting surface.
 ...
 \begin{figure*}[t]
  \centerline{\begin{overpic}[width=0.25\textwidth]{fig/cheese.jpg}
		\put(0,5){(a)}
	\end{overpic}\relax
	\begin{overpic}[width=0.25\textwidth]{fig/cheese-n.jpg}
		\put(0,5){(b)}
	\end{overpic}\relax
	\begin{overpic}[width=0.25\textwidth]{fig/cheeseforces.jpg}
		\put(0,5){(c)}
	\end{overpic}\hfill
...
(b). The magnitude of forces in the edges of this thrust\dash
network is visualized by the cross\dash section of edges
in (c).}\label{fig:cheese2}
	\end{minipage}}
	\end{figure*}
...
plausible that the surface is no longer self\dash supporting.}

\vspace{4cm}
\caption{Test sequence: Stability w.r.t.\ increasing loads in a single
point. \todo{IMAGES SIMILAR TO PREVIOUS}}
...
shape, joints will experience low bending and torsion moments.}
