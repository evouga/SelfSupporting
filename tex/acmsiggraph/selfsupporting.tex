...
\TOGarticleDOI{10.1145/2185520.2185577}
...
$2\times 2$ symmetric positive semidefinite matrices satisfying}
	...
	\Div M &= 0,
	  \label{eq:conds}
	\end{align}
 \newtext{where $g$ is the induced metric ${1+s_x^2 \quad s_x s_y \choose s_x s_y \quad 1+s_y^2}$ and} the divergence operator $\Div{u(x,y)\choose v(x,y}= u_x + v_y$ is
...
\end{table}
 ...
 \begin{figure*}[t]
 \centerline{\begin{overpic}[width=.2\textwidth]{fig/video1}\put(0,10){(a)}
     \end{overpic}\relax
 \begin{overpic}[width=.23\textwidth]{fig/video3}\put(0,10){(b)}\end{overpic}\relax
 \begin{overpic}[width=.23\textwidth]{fig/video4}\put(0,10){(c)}\end{overpic}\hfill
...
Fig.~\protect\ref{fig:teaser}).}
	...
	\includegraphics[height=0.19\textwidth]{arch-fig/1roo58.jpg}
  \end{figure*}
