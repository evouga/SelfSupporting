...
\caption{The user-designed reference mesh (left) is not self-supporting, 
but our algorithm finds a nearby perturbation of the reference surface 
(middle-left) that is in equilibrium. As the user makes edits to the 
reference surface (middle-right), the thrust network automatically adjusts 
(right). \label{fig:vault}}
...
%\section*{Acknowledgements}
  ...
  \centerline{\includegraphics[width=2.57\tmplen]{fig/remove-1n.jpg}\hfill
	  ...
	  \raise.43\tmplen\hbox{\includegraphics[width=2.23\tmplen]
			{fig/remove-3n.jpg}}\hfill
	  \raise.43\tmplen\hbox{\includegraphics[width=2.24\tmplen]
			{fig/remove-4n.jpg}}}
   \vskip-6ex
...
\caption{Destruction sequence. We simulate removing a part of masonry and 
the falling off of further pieces which are no longer supported after 
removal. This is done by deleting the 1-neighborhood of a vertex (shown in 
yellow) and solving for a new thrust network in compressive equilibrium 
...
hull, and iterate. (a) For this example, removing a certain small number 
single bricks does not affect stability. (b) Removal of material at a 
certain point will cause a greater part of the structure to collapse, as 
seen in (c). (d) shows the result after one more removal (all images show 
...
\centerline{\begin{minipage}[b]{0.8\columnwidth}
	\centering
...
load of 900\,kg (shown in black) is applied to a certain vertex. This 
...
This method of testing is rather conservative).}\label{fig:load}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{1.1\columnwidth}
...
necessarily self\dash supporting itself). Since this surface is ivery 
