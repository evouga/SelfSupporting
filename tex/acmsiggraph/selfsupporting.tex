...
establish a connection with mean curvatures which are otherwise undefined
...
This section is the theoretical basis of the paper. Subsections
\ref{ssec:cont} and \ref{sec:thrustnetworks} explain the mathematical
model we use and its discretization, which is essential for our
modeling algorithms with self\dash supporting surfaces. The connection
...
variational problem. Our experience with black-box solvers~\cite{ipopt} is that
...
\end{table}
  ...
  \begin{figure*}[t]
  \newdimen\tmplen\tmplen=0.09\textwidth
...
We can, however, make a few remarks. Only Step ~\ref{step2} can increase the
equilibrium
...
Figure~\protect\ref{fig:Lilium}. (a) Relative principal directions which are found
...
supporting. (c) Small changes achieve both properties.}
 \label{fig:lilium:pq}\vspace*{1em}\end{minipage}
  \vspace*{-1em}
\end{figure*}
...
\ref{fig:Lilium}). What is we had wanted to build this surface out of masonry instead?
This surface contains a concave part with local minimum
in its interior and so cannot possibly be self-supporting without modification. Given this
...
Figures~\ref{fig:cheese} and \ref{fig:cheese2}b).
