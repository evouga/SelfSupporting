...
%%% ``\begin{document}'' and ``\maketitle.''
 ...
 \teaser{\centerline{%{arch-fig/1roo64.jpg}}
	\includegraphics[height=.30\textwidth]{arch-fig/cheesevault78.jpg}\hfill
	\includegraphics[height=.30\textwidth]{arch-fig/1roo97}}

	\caption{{\em Left:} Surfaces with irregularly placed holes
are hard to realize as masonry, where the mortar between bricks
must not be subject to tensile stresses. The surface shown here has
this surprising property -- it has been found as the nearest
self\dash supporting shape from a given freeform geometry.
The fictitious thrust network used in our algorithms is also shown, with 
edges' cross-section and coloring visualizing the magnitude of
forces (warmer colors represent higher stresses.)
{\em Right:} Curvature analysis with respect the so\dash called Airy stress
surface
tells us how to remesh shapes by self\dash supporting quad meshes
with planar faces, which guide steel\slash glass constructions with
...
shapes by self\dash supporting quad meshes with planar faces. This leads
...
moment\dash free steel\slash glass constructions. Finally
\S\ref{ssec:convergence} deals with the question how faithful our
discretizations are.
...
\end{table}
	...
	\begin{figure*}[t]
	\centerline{\includegraphics[height=0.14\textwidth]{fig/cas.jpg}\hfill
...
\bigskip
  ...
  \newdimen\tmplen\tmplen=0.12\textwidth
...
\label{fig:load}
	...
	\begin{overpic}[width=.49\columnwidth]{fig/dome-unloaded.jpg}
		...
		\put(0,50){shell thickness 0.1\,m, $\rho=2,500$\,kg/m$^3$}
	\end{overpic}\hfill
...
\bigskip
  \centerline{\begin{overpic}[width=0.33\textwidth]{fig/cheese.jpg}
		\put(0,5){(a)}
	\end{overpic}\relax
	\begin{overpic}[width=0.33\textwidth]{fig/cheese-n.jpg}
		\put(0,5){(b)}
	\end{overpic}\hfill
...
\sparagraph{Example: Stability Test:} See Figures \ref{fig:load} and
...
\centerline{\raise0.02\textwidth\hbox{\begin{overpic}
		[width=.28\textwidth]{fig/lilium-vf.jpg}
		\put(0,0){(a)}
	\end{overpic}}\hfill
	\raise0.02\textwidth\hbox{\begin{overpic}
		[width=.28\textwidth]{fig/lilium-pq.jpg}
		\put(0,0){(b)}
	\end{overpic}}\hfill
	\raise0.02\textwidth\hbox{\begin{overpic}
		[width=.26\textwidth]{arch-fig/lilium-pq-93.jpg}
		\put(0,0){(c)}
	\end{overpic}}}
...
Fig.~\protect\ref{fig:teaser}).}
	...
	\includegraphics[height=0.20\textwidth]{arch-fig/1roo58.jpg}
  \end{figure*}
...
\ref{fig:load}) have real\dash world analogues on his web page \cite{catalan}.
 ...
 {\begin{overpic}[width=.52\textwidth]{arch-fig/schneider91.jpg}
 	\put(100,-33){{\includegraphics[width=\columnwidth]
